"""Core AI orchestrator for support draft generation."""
...
class SupportCopilot:
    """Orchestrates memory, RAG, tool calls, and LLM response drafting."""
    ...
    def __init__(self, settings: Settings):
        ...
        self._tool_enabled_llm = self._llm.bind_tools(self._tools)
        ...
        try:
            ...
            self.memory = NoopMemoryStore(reason=self._memory_error)
    ...
    def generate_draft(self, ticket: dict[str, Any], customer: dict[str, Any]) -> dict[str, Any]:
        ...
        for _ in range(max_tool_rounds):
            response = self._tool_enabled_llm.invoke(messages)
            final_response = response
            messages.append(response)

            pending_calls = getattr(response, "tool_calls", None) or []
            ...
            for call in pending_calls:
                ...
                if not tool:
                    trace.update(
                        {
                            "status": "skipped",
                            "summary": f"Tool '{safe_tool_name}' was requested but is unavailable.",
                            "output": None,
                            "output_text": f"Tool '{safe_tool_name}' unavailable.",
                        }
                    )
                    tool_calls.append(trace)
                    if tool_id:
                        messages.append(ToolMessage(content=trace["output_text"], tool_call_id=tool_id))
                    continue
                ...
                try:
                    ...
                    output_text = f"Tool execution failed for {safe_tool_name}: {exc}"
                    trace.update(
                        {
                            "status": "error",
                            "summary": output_text,
        ...
        draft_text = self._extract_content(final_response).strip()
        used_fallback = False
        if not draft_text:
